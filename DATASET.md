# DATASET â€” Polymarket order book, 18.4 days

**English** Â· [PortuguÃªs](DATASET.pt-BR.md)
...
curl -LO $B/book_top.parquet        # 123 MB â€” the order book
curl -LO $B/wallet_trades.parquet   #  41 MB â€” trades of the watched wallets
curl -LO $B/markets.parquet         # 1.2 MB â€” catalogue, fees, resolution
```
...
- **`ts_local` is the collector's clock**, `ts_exchange` is the exchange's. The
  gap between them is network delay â€” and it is what decided this project.
...
- **`wallet_trades.ts_seen - ts_trade`** is the delay before a trade appears on
  the public API. Median 335s â€” the number that killed the copy-trading thesis.
